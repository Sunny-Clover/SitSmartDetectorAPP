...
func convertPixelBufferToBGRA(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
    ...
    guard status == kCVReturnSuccess, let outputPixelBuffer = bgraPixelBuffer else {
        print("Failed to create BGRA pixel buffer.")
        return nil
    }
    
    CVPixelBufferLockBaseAddress(outputPixelBuffer, .readOnly)
    guard let baseAddress = CVPixelBufferGetBaseAddress(outputPixelBuffer) else {
        print("Failed to get base address of BGRA pixel buffer.")
