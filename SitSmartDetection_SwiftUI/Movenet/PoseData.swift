...
extension Person {
    /// 将 `Person` 中的关键点信息转换为包含所有 `x`, `y`, 和 `score` 的一维数组
    func toFlattenedArray() -> [Float32] {
        var flattenedArray: [Float32] = []
        
        for keyPoint in keyPoints {
            flattenedArray.append(Float32(keyPoint.coordinate.x))
            flattenedArray.append(Float32(keyPoint.coordinate.y))
            flattenedArray.append(keyPoint.score)
        }
