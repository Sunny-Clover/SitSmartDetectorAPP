...
final class MoveNet: PoseEstimator {
  ...
  /// - Returns: The cropped and resized image. `nil` if it can not be processed.
    private func preprocess(_ pixelBuffer: CVPixelBuffer) -> Data? {
        guard let convertedPixelBuffer = convertPixelBufferToBGRA(pixelBuffer) else {
            os_log("Failed to convert pixel buffer to 32BGRA format.", type: .error)
            return nil
        }
        
        let sourcePixelFormat = CVPixelBufferGetPixelFormatType(convertedPixelBuffer)
        if !(sourcePixelFormat == kCVPixelFormatType_32BGRA || sourcePixelFormat == kCVPixelFormatType_32ARGB) {
            os_log("Unexpected pixel format: %d", type: .error, sourcePixelFormat)
            return nil
        }
        ...
        let imageWidth = CVPixelBufferGetWidth(convertedPixelBuffer)
        let imageHeight = CVPixelBufferGetHeight(convertedPixelBuffer)

        let cropRegion = self.cropRegion ?? initialCropRegion(imageWidth: CGFloat(imageWidth), imageHeight: CGFloat(imageHeight))
        self.cropRegion = cropRegion

        let rectF = RectF(
            left: cropRegion.left * CGFloat(imageWidth),
            top: cropRegion.top * CGFloat(imageHeight),
            right: cropRegion.right * CGFloat(imageWidth),
            bottom: cropRegion.bottom * CGFloat(imageHeight)
        )

        // Detect region
        let modelSize = CGSize(width: inputWidth, height: inputHeight)
        guard let thumbnail = convertedPixelBuffer.cropAndResize(fromRect: rectF.rect, toSize: modelSize) else {
            os_log("Failed to crop and resize the image.", type: .error)
            return nil
        }
  ...
  private func determineTorsoAndBodyDistances(
    ...
    func convertPixelBufferTo32BGRA(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        let ciImage = CIImage(cvPixelBuffer: pixelBuffer)
        let context = CIContext()
        let options: [CIImageOption: Any] = [.colorSpace: CGColorSpaceCreateDeviceRGB()]

        guard let cgImage = context.createCGImage(ciImage, from: ciImage.extent, format: .BGRA8, colorSpace: CGColorSpaceCreateDeviceRGB()) else {
            return nil
        }
        ...
        guard status == kCVReturnSuccess, let outputPixelBuffer = bgraPixelBuffer else {
            return nil
        }
        ...
        context2?.draw(cgImage, in: CGRect(x: 0, y: 0, width: CGFloat(cgImage.width), height: CGFloat(cgImage.height)))
